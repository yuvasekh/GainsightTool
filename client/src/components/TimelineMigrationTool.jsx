...
import { ChevronDown, Database, Play, CheckCircle, AlertCircle, Loader, ArrowRight, Clock, RefreshCw, Eye, EyeOff } from 'lucide-react';
...
useEffect(()=>
{},[sourceFields])
 const generateTimelines = async (sourceUrl, sourceToken, sourceObject, targetUrl, targetToken, targetObject, fields) => {
 try {
 ...
 const startMigration = async (timelines) => {
 try {
...
// var response=await fetchFieldNames(sourceUrl,sourceToken,objectId)
//  if (!objectId) {
//  setSourceFields([]);
//  return;
//  }
...
//  { id: 'created_date', name: 'Created Date', type: 'datetime', required: false },
//  { id: 'modified_date', name: 'Modified Date', type: 'datetime', required: false },
//  { id: 'status', name: 'Status', type: 'picklist', required: false }
//  ];
 ...
 alert('No timelines to migrate');
 return;
 }
console.log(targetUrl,targetToken,selectedTargetObject,sourceUrl,sourceToken,selectedSourceObject,selectedFields);
 ...
 // In real implementation, use: await startMigration(timelines);
 ...
 >
 {showSourceToken ? <EyeOff className="w-4 h-4 text-gray-400" /> : <Eye className="w-4 h-4 text-gray-400" />}
 </button>
 </div>
 </div>
 </div>
 ...
 className="absolute right-3 top-1/2 transform -translate-y-1/2"
 >
 {showTargetToken ? <EyeOff className="w-4 h-4 text-gray-400" /> : <Eye className="w-4 h-4 text-gray-400" />}
 </button>
 </div>
 </div>
 </div>
 ...
 timeline.status === 'running' ? 'bg-blue-100 text-blue-800' :
